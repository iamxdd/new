App.controller('classificationSettingsCtrl', ['$scope', '$http', 'ngDialog', 'PagerExtends', 'layerAlert','serverUrls','PcService', function($scope, $http, ngDialog, PagerExtends, layerAlert,serverUrls,PcService) {
	$scope.list = [];
   ...
   // zTree 的参数配置，深入使用请参考 API 文档（setting 配置详解）
    ...
    function beforeRemove(treeId, treeNode) {
    	var zTree = $.fn.zTree.getZTreeObj("treeDemo");
    	var url='';
    	url=serverUrls.updatename+'?id='+treeNode.id
	...
	function addHoverDom(treeId, treeNode) {
		if(treeNode.level==0){
			...
			if (btn) btn.bind("click", function(){
				...
				indexId=0;
				urlId=1; 
				creatNewsTypes('新增新闻分类',serverUrls.updatename);
				return false;
			});
		}
	...
	function beforeEditName(treeId, treeNode) { 
	    ...
	    var zTree = $.fn.zTree.getZTreeObj("treeDemo");  
	    zTree.selectNode(treeNode);  
	    creatNewsTypes('修改新闻分类'+editName,serverUrls.updatename);
        return false;	
	} 
		...
		controller: ['$scope', function($scope) {
           ...
           var banner=[{
			...
			$scope.Adnews=[
			{
			 ...
			 {
				name: "PopulationType1",
				nameDisplay: "banner",
				editor: "multiselect",
				required: true,
				value: "",
				opts: banner,
				originValue: banner[0].Id,
				editable:true
			 }];
			$scope.formSubmit = function() {
				var method='post',url=serverUrls.DeleteAdvertisement;
				var newtypedata=[];
